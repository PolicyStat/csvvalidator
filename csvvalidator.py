...
class CSVValidator(object):
    ...
    def add_record_length_check(
        ...
        """
        Add a record length check, i.e., check whether the length of a record
        is consistent with the number of expected fields.
    ...
    def add_value_check(self, field_name, value_check,
        ...
        # guard conditions
        assert field_name in self._field_names, 'unexpected field name: %s' % field_name  # noqa
    ...
    def add_value_predicate(
        ...
        assert field_name in self._field_names, 'unexpected field name: %s' % field_name  # noqa
        assert callable(value_predicate), 'value predicate must be a callable function'  # noqa
    ...
    def add_record_check(self, record_check, modulus=1):
        ...
        assert callable(record_check), 'record check must be a callable function'  # noqa
    ...
    def add_record_predicate(
        ...
        N.B., everything you can do with record predicates can also be done
        with record check functions, whether you use one or the other is a
        matter of style.

        Arguments
        ---------

        `record_predicate` - a function that accepts a single argument (a
        record as a dictionary of values indexed by field name) and returns
        ...
        assert callable(record_predicate), 'record predicate must be a callable function'  # noqa
    ...
    def add_unique_check(
        ...
        if isinstance(key, six.string_types):
            ...
            for f in key:
                assert f in self._field_names, 'unexpected field name: %s' % key  # noqa
        t = key, code, message
    ...
    def validate(self, data,
        ...
        `report_unexpected_exceptions` - value check function, value
        predicates, record check functions, record predicates, and other
        user-supplied validation functions may raise unexpected exceptions. If
        this argument is true, any unexpected exceptions will be reported as
        validation problems; if False, unexpected exceptions will be handled
        silently.
    ...
    def ivalidate(
        ...
        for i, r in enumerate(data):
            if expect_header_row and i == ignore_lines:
                ...
                # r is a data row
                skip = False
                for p in self._apply_skips(i, r, summarize, report_unexpected_exceptions, context):  # noqa
                    if p is True:
                        skip = True
                    else:
                        yield p
                if not skip:
                    for p in self._apply_each_methods(i, r, summarize, report_unexpected_exceptions, context):  # noqa
                        # may yield a problem if an exception is raised
                        yield p
                    for p in self._apply_value_checks(i, r, summarize, report_unexpected_exceptions, context):  # noqa
                        yield p
                    for p in self._apply_record_length_checks(i, r, summarize,
                                                              context):
                        yield p
                    for p in self._apply_value_predicates(i, r, summarize, report_unexpected_exceptions, context):  # noqa
                        yield p
                    for p in self._apply_record_checks(i, r, summarize, report_unexpected_exceptions, context):  # noqa
                        yield p
                    for p in self._apply_record_predicates(i, r, summarize, report_unexpected_exceptions, context):  # noqa
                        yield p
                    for p in self._apply_unique_checks(i, r, unique_sets, summarize):  # noqa
                        yield p
                    for p in self._apply_check_methods(i, r, summarize, report_unexpected_exceptions, context):  # noqa
                        yield p
                    for p in self._apply_assert_methods(i, r, summarize, report_unexpected_exceptions, context):  # noqa
                        yield p
        for p in self._apply_finally_assert_methods(summarize, report_unexpected_exceptions, context):  # noqa
            yield p
                ...
                if fi < len(r):  # only apply checks if there is a value
                    ...
                    try:
                        ...
                        if report_unexpected_exceptions:
                            ...
                            if not summarize:
                                p['message'] = MESSAGES[UNEXPECTED_EXCEPTION] % (e.__class__.__name__, e)  # noqa
                                p['row'] = i + 1
                                p['column'] = fi + 1
                                p['field'] = field_name
                                p['value'] = value
    ...
    def _apply_value_predicates(self, i, r,
        ...
        for field_name, predicate, code, message, modulus in self._value_predicates:  # noqa
            if i % modulus == 0:  # support sampling
                fi = self._field_names.index(field_name)
        ...
        for check, modulus in self._record_checks:
            if i % modulus == 0:  # support sampling
                ...
                try:
                    ...
                    code = e.code if e.code is not None else RECORD_CHECK_FAILED   # noqa
                    p = {'code': code}
                    if not summarize:
                        message = e.message if e.message is not None else MESSAGES[RECORD_CHECK_FAILED]   # noqa
                        p['message'] = message
                        p['row'] = i + 1
                        p['record'] = r
                        if context is not None:
        ...
        for a in dir(self):
            if a.startswith('assert'):
                ...
                try:
                    ...
                    code = e.args[0] if len(e.args) > 0 else ASSERT_CHECK_FAILED  # noqa
                    p = {'code': code}
                    if not summarize:
                        message = e.args[1] if len(e.args) > 1 else MESSAGES[ASSERT_CHECK_FAILED]  # noqa
                        p['message'] = message
                        p['row'] = i + 1
                        p['record'] = r
                        if context is not None:
        ...
        for a in dir(self):
            if a.startswith('finally_assert'):
                ...
                try:
                    ...
                    code = e.args[0] if len(e.args) > 0 else FINALLY_ASSERT_CHECK_FAILED  # noqa
                    p = {'code': code}
                    if not summarize:
                        message = e.args[1] if len(e.args) > 1 else MESSAGES[FINALLY_ASSERT_CHECK_FAILED]  # noqa
                        p['message'] = message
                        if context is not None:
                            p['context'] = context
                    yield p
...
def enumeration(*args):
    """
    Return a value check function which raises a value error if the value is
    not in a pre-defined enumeration of values.
    ...
    If you pass in more than on argument, it is assumed the arguments
    themselves define the enumeration.
...
""")
        ...
        if not summarize:
            ...
            underline += '\n'
            w(underline)
            for k in sorted(set(p.keys()) - {'code', 'message', 'context'}):
                w(':%s: %s\n' % (k, p[k]))
            if 'context' in p:
                c = p['context']
